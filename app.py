...
app = Flask(__name__)
app.config.from_pyfile('config.py') # 引入配置项
...
# -----admin-----
from myadmin import *
admin = Admin(app, name='网上书店后台', index_view=MyAdminIndexView())
...
def index():
    ...
    books = Book.query.filter(Book.state == "正在出售").order_by(db.desc(Book.id)).all()
...
def hotsort():
    ...
    books = Book.query.filter(Book.state == "正在出售").all()
...
def search():
    ...
    if searchform.validate_on_submit():
        ...
        print(content)
        books = Book.query.filter(or_(Book.id.like('%'+content+'%'), Book.bookname.like('%'+content+'%'), Book.detail.like('%'+content+'%')), Book.state == "正在出售").all()
...
def searchtag():
    ...
    if searchform.validate_on_submit():
        content = searchform.content.data
        print(content)
        books = Book.query.filter(Book.tag == content, Book.state == "正在出售").all()
...
def mysell():
    form = BuyBookForm()
    books = Book.query.filter(Book.seller_id == current_user.id, Book.state == "正在出售").all()
...
def mysell_delete():
    form = BuyBookForm()
    if form.validate_on_submit():
        orders = Order.query.filter(Order.book_id == form.book_id.data, Order.seller_id == current_user.id, Order.state == "收藏").all()
        for order in orders:
